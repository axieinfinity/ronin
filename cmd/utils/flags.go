...
var (
	...
	PyroscopeEnableFlag = &cli.BoolFlag{
		...
		Category: flags.ProfilingCategory,
	}
...
func SetNodeConfig(ctx *cli.Context, cfg *node.Config) {
	...
	if ctx.IsSet(EnableSigningMethodsFlag.Name) {
		cfg.EnableSigningMethods = ctx.Bool(EnableSigningMethodsFlag.Name)
	}
...
func setFastFinality(ctx *cli.Context, cfg *node.Config) {
	cfg.MaxCurVoteAmountPerBlock = ctx.Int(MaxCurVoteAmountPerBlock.Name)
	cfg.EnableFastFinality = ctx.Bool(EnableFastFinality.Name)
	cfg.EnableFastFinalitySign = ctx.Bool(EnableFastFinalitySign.Name)
	cfg.BlsPasswordPath = ctx.String(BlsPasswordPath.Name)
	cfg.BlsWalletPath = ctx.String(BlsWalletPath.Name)
}
...
func setMiner(ctx *cli.Context, cfg *miner.Config) {
	...
	if ctx.IsSet(MinerBlockProduceLeftoverFlag.Name) {
		cfg.BlockProduceLeftOver = ctx.Duration(MinerBlockProduceLeftoverFlag.Name)
	}
	cfg.BlockSizeReserve = ctx.Uint64(MinerBlockSizeReserveFlag.Name)
...
func SetEthConfig(ctx *cli.Context, stack *node.Node, cfg *ethconfig.Config) {
	...
	cfg.TriesInMemory = ctx.Int(TriesInMemoryFlag.Name)
	...
	if ctx.IsSet(DisableRoninProtocol.Name) {
		cfg.DisableRoninProtocol = ctx.Bool(DisableRoninProtocol.Name)
	}
	...
	if ctx.Bool(MonitorDoubleSign.Name) {
		cfg.EnableMonitorDoubleSign = true
	}

	if ctx.Bool(AdditionalChainEventFlag.Name) {
...
func RegisterReadinessService(stack *node.Node, ctx *cli.Context) {
	if err := NewReadinessHandler(stack, []string{"*"}, []string{"*"}, ctx.String(ReadinessPrometheusEndpointFlag.Name), ctx.Int64(ReadinessBlockLagFlag.Name)); err != nil {
