		...
		go func() {
			...
			for task := range taskCh {
				...
				for i, tx := range task.block.Transactions() {
					...
					task.results[i] = &txTraceResult{TransactionHash: tx.Hash(), Result: res}
				}
...
func (api *API) traceBlock(ctx context.Context, block *types.Block, config *TraceConfig) ([]*txTraceResult, error) {
	...
	for i, tx := range txs {
		...
		results[i] = &txTraceResult{TransactionHash: tx.Hash(), Result: res}
	...
	for th := 0; th < threads; th++ {
		...
		go func() {
			...
			for task := range jobs {
				...
				results[task.index] = &txTraceResult{TransactionHash: txs[task.index].Hash(), Result: res}
			}
		}()
	}
