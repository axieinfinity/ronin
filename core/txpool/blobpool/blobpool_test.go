...
func init() {
	...
	*testChainConfig = *params.MainnetChainConfig
	testChainConfig.Consortium = &params.ConsortiumConfig{
		EpochV2: 200,
	}
	testChainConfig.OdysseusBlock = common.Big0
	testChainConfig.CancunBlock = common.Big0
}
...
func TestSponsoredTxRejection(t *testing.T) {
	statedb, _ := state.New(types.EmptyRootHash, state.NewDatabase(rawdb.NewDatabase(memorydb.New())), nil)
	...
	errs := pool.Add([]*types.Transaction{tx}, false, true)
	if !errors.Is(errs[0], core.ErrTxTypeNotSupported) {
		t.Fatalf("Expect error %v got %v", core.ErrTxTypeNotSupported, errs[0])
	}
}
