...
//
//   - For the scenario where the pool does not con	chainconfig *params.ChainConfigll need to be done only starting at the swapped in/out nonce
...
func New(config Config, chainConfig *params.ChainConfig, chain BlockChain) *BlobPool {
		...
		for addr, txs := range p.index {
			for i, tx := range txs {
				...
				if p.chainConfig.IsVenoki(p.head.Number) {
					...
					maxTip := new(uint256.Int).Sub(tx.execFeeCap, uint256.NewInt(params.MinimumBaseFee))
					feeCapUnderpriced = maxTip.Cmp(p.gasTip) < 0
				}
...
func (p *BlobPool) Pending(filter *txpool.PendingFilter) map[common.Address][]*txpool.LazyTransaction {
	...
	for addr, txs := range p.index {
		...
		for _, tx := range txs {
			...
			if filter.EnforceTip && filter.BaseFee != nil {
